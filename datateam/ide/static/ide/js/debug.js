...
function debug_ui (toggle) {
  ...
  if (toggle == 'replace_next' || toggle == 'prev_next' || toggle == 'replace_all_tabs' || toggle == 'debug_status') {
  ...
  if (toggle == 'debug_status' || toggle == 'replace_status') {
    $("#debug_status").css("display", "block");
    
    if (toggle == 'debug_status') {
      $("#debug_status span").html('Searching ...');
    }
...
function do_search () {
  ...
  if (needle == '' && sin != 'dir') {
    ...
    debug_options['needle'] = needle;
    
    if (sin == 'current') {
      current_search = false;
      if (stype == 'replace') {
        debug_ui('replace_next');
      }
      ...
      var html = '<div class="title"><em>All Tabs</em><strong>Search For: ' + debug_options['needle'] + '</strong></div>';
      html += '<table>';
      ...
      if (stype == 'search' && needle == '' && glob == '') {
        ...
        $.ajax({
           ...
           success: function (data, textStatus, jqXHR) {
             searchWorker = setTimeout(function () { check_debug_status(data.task_id, data.dsid) }, 500);
           },
  ...
  $.ajax({
     ...
     success: function (data, textStatus, jqXHR) {
       ...
       if (data.state == 'complete') {
         ...
         searchWorker = setTimeout(function () { check_debug_status(task_id, dsid) }, 3000);
       }
     },
     error: function (jqXHR, textStatus, errorThrown) {
       searchWorker = setTimeout(function () { check_debug_status(task_id, dsid) }, 3000);
...
function set_results_dir_search(task_id, dsid, results) {
  if (dir_results_started) {
    ...
    var html = '<div class="title"><em>Directory</em><strong>Search For: ' + debug_options['needle'] + '</strong></div>';
    html += '<table>';
    html += '<tr><td><strong>Filename</strong></td><td><strong>Matches</strong></td></tr>';
...
function debug_next (way) {
  ...
  if (current_range) {
    ...
    alert("The search of a thousand miles has completed.");
  }
}
