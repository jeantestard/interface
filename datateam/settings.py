...
SESSION_COOKIE_NAME = 'datateamsession'
...
BG_IMG = 'ide/img/datateam.jpg'

import djcelery
djcelery.setup_loader()

try:
  from datateam.settings_local import *
...
if not globals().has_key('WORKING_DIR'):
  WORKING_DIR = os.path.join(os.environ['HOME'], 'datateam')
...
if not globals().has_key('LOGPATH'):
  if DEBUG:
    LOGPATH = os.path.join(LOG_DIR, 'datateamide_debug.log')
    
  else:
    LOGPATH = os.path.join(LOG_DIR, 'datateamide.log')
  
if not globals().has_key('PKLPATH'):
  PKLPATH = os.path.join(WORKING_DIR, 'datateamide.pkl')
...
if not globals().has_key('DATABASES'):
  DATABASES = {
      'default': {
          ...
          'NAME': os.path.join(WORKING_DIR, 'datateam.sql3'),                      # Or path to database file if using sqlite3.
